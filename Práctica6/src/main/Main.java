...
import adaptadores.AdaptadorFoto;
import adaptadores.AdaptadorMonumento;
...
public class Main {
	...
	public static void main(String[] args) {
		...
		for (Monumento monumento : db.selectMonumentos()) {
			 map.add(new AdaptadorMonumento(monumento.getNombre(), monumento.getAutor(), monumento.getDirección()));
		}

		for (Foto foto : db.selectFotos()) {
			 map.add(new AdaptadorFoto(foto.getUsuario(), foto.getDescripción(), foto.getCoordenadas()));
		}

		for (Restaurante restaurante : db.selectRestaurantes()) {
			map.add(new AdaptadorRestaurante(restaurante.getNombre(), restaurante.getDirección(), restaurante.getTeléfono()));
		}
